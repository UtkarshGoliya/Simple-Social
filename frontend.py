            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    if isinstance(detail, str):
                        ...
                        first = detail[0]
                        msg = first.get("msg", str(first)) if isinstance(first, dict) else str(detail)
                    else:
                        msg = str(detail) if detail else "Upload failed!"
                except Exception:
                    msg = f"Upload failed (HTTP {response.status_code})"
